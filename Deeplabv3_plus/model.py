"""Hand-implemented DeepLabV3+ for CamVid semantic segmentation.

Architecture
------------
Backbone  : ResNet-101 (optionally ImageNet-pretrained, output_stride=16)
ASPP      : 1x1 + 3x3@rate6/12/18 + global-avg-pool  (256 ch each)
Decoder   : low-level features (layer1, 1/4 res) fused with ASPP output
Classifier: depthwise-sep 3x3 -> 3x3 -> 1x1
Upsample  : bilinear x4 back to input resolution
"""
...
class _DepthwiseSepConv(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, padding: int = 1) -> None:
        super().__init__()
        self.dw = nn.Conv2d(in_ch, in_ch, 3, padding=padding, groups=in_ch, bias=False)
...
class DeepLabV3PlusDecoder(nn.Module):
    def __init__(self, low_ch: int, aspp_ch: int, num_classes: int) -> None:
        ...
        self.refine = nn.Sequential(
            _DepthwiseSepConv(aspp_ch + 48, 256),
...
class CustomDeepLabV3(nn.Module):
    ...
    - ResNet-101 backbone (deeper, better features)
    - DeepLabV3+ decoder (low-level feature fusion at 1/4 resolution)
    - Auxiliary head on layer3 for extra gradient signal
    - Backbone stem+layer1 frozen to reduce overfitting
    """
